...
export default function UnifiedOverview() {
  ...
  useEffect(() => {
    (async () => {
      const params = new URLSearchParams({ commodity: "Wheat", state: "Punjab" });
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-4">
        <div className="rounded-md border border-slate-200 p-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><Sprout className="h-4 w-4 text-emerald-600"/> Soil</div>
          <div className="mt-2 text-sm text-slate-700">
            <div>Farmer: {farmer?.name || "—"}</div>
            <div>Soil type: {farmer?.soilType || "Unknown"}</div>
            <div>Land size: {farmer?.landSize ? `${farmer.landSize} acres` : "—"}</div>
            <a href="/login" className="mt-2 inline-block text-xs text-emerald-700 underline">Update profile</a>
          </div>
        </div>
        <div className="rounded-md border border-slate-200 p-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><CloudSun className="h-4 w-4 text-amber-600"/> Weather</div>
          <div className="mt-2 text-sm text-slate-700">
            {weather ? (
              <>
              ...
              <div className="text-slate-500">{weatherError || "Allow location for local weather."}</div>
            )}
          </div>
        </div>
        <div className="rounded-md border border-slate-200 p-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><Store className="h-4 w-4 text-blue-600"/> Market</div>
          <div className="mt-2 text-sm text-slate-700">
            {market.length ? (
              <ul className="space-y-1">
                {market.map((m, i) => (
                  <li key={i} className="flex justify-between"><span>{m.mandi}</span><span className="font-medium">₹ {m.price}</span></li>
                ))}
              </ul>
            ) : (
              <div className="text-slate-500">No data</div>
            )}
            <a href="#market" className="mt-2 inline-block text-xs text-blue-700 underline">Open market & weather</a>
          </div>
        </div>
        <div className="rounded-md border border-slate-200 p-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><Bug className="h-4 w-4 text-rose-600"/> Pest</div>
          <div className="mt-2 text-sm text-slate-700">
            <div>Upload a leaf/crop image to analyze.</div>
            <a href="#pest" className="mt-2 inline-block text-xs text-rose-700 underline">Open detector</a>
          </div>
        </div>
      </div>
    </div>
